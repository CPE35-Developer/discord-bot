...
def get_random_cards(players) -> Tuple[List[Tuple[int, int]], List[int]]:
    ...
    for idx in range(0, len(player_cards), 3):
        triple_cards = tuple(
            (player_cards[idx], player_cards[idx+1], player_cards[idx+2]))
...
async def call_card(bot, ctx, players, player_cards):
...
async def score_cards(players, player_status, player_cards):
    ...
    # score=[ป็อก,ความพิเศษ,แต้ม,เด้ง,id]
    score = [[0, 0, 0, 0, 0]] * len(players)
    dic = {":a:": 1,
           ":two:": 2,
           ":three:": 3,
           ":four:": 4,
    ...
    for i in range(len(players)):
        ...
        if player_status[i] == 'p':
            ...
            sumofnum = (int(dic[cards[0][0]])) + \
                int(dic[(cards[1][0])])+int(dic[(cards[2][0])])
            score[i][2] = sumofnum % 10
            sortcard = sorted(
                list((dic2[cards[0][0]], dic2[cards[1][0]], dic2[cards[2][0]])))
            if [cards[0][0], cards[1][0], cards[2][0]] in [":regional_indicator_j:", ":regional_indicator_q:", ":regional_indicator_k:"]:
                score[i][1] = 3
            elif (sortcard[2]-sortcard[0] == 2 and sortcard[0] != sortcard[1] != sortcard[2]):
                score[i][1] = 3
...
async def check_winner(ctx, players, player_status, index_of_bighand, listscore, player_cards):
    if player_status[int(index_of_bighand)] == "p":
        await ctx.channel.send(f"เจ้ามือถือไพ่ {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][0]]}   {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][1]]}")
    else:
        await ctx.channel.send(f"เจ้ามือถือไพ่ {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][0]]}   {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][1]]}   {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][2]]}")
    for i in range(len(players)):
        ...
        if listscore[i] > listscore[int(index_of_bighand)]:
            if player_status[i] == "p":
                await ctx.send(f"{str(players[i])} ชนะเจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}")
            elif player_status[i] == "c":
                await ctx.channel.send(f"{str(players[i])} ชนะเจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}   {DECKS_OF_CARDS[player_cards[i][2]]}")
        elif listscore[i] < listscore[int(index_of_bighand)]:
            if player_status[i] == "p":
                await ctx.send(f"{str(players[i])} แพ้เจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}")
            elif player_status[i] == "c":
                await ctx.channel.send(f"{str(players[i])} แพ้เจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}   {DECKS_OF_CARDS[player_cards[i][2]]}")
        else:
            if player_status[i] == "p":
                await ctx.send(f"{str(players[i])} เสมอกับเจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}")
            elif player_status[i] == "c":
                await ctx.channel.send(f"{str(players[i])} เสมอกับเจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}   {DECKS_OF_CARDS[player_cards[i][2]]}")
    return
...
async def replay(bot, ctx, players, player_cards, player_status, index_of_bighand, listscore):
    ...
    while True:
        ...
        if (chk_msg.content.lower() == 'y'):
            ...
            player_status = await call_card(bot, ctx, players, player_cards)
