...
async def score_cards(players,player_status,player_cards):
    ...
    score =[[0,0,0,0,0]]* len(players)#score=[ป็อก,ความพิเศษ,แต้ม,เด้ง,id]
    dic = { ":a:": 1,
        ":two:": 2,
        ":three:": 3,
        ":four:": 4,
...
async def check_winner(ctx, players,player_status,index_of_bighand,listscore,player_cards):
    ...
    else: await ctx.channel.send(f"เจ้ามือถือไพ่ {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][0]]}   {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][1]]}   {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][2]]}")
    for i in range(len(players)):
        if i == int(index_of_bighand):continue
        
        if listscore[i]>listscore[int(index_of_bighand)]: 
            if player_status[i]=="p":await ctx.send(f"{str(players[i])} ชนะเจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}")
            ...
            if player_status[i]=="p":await ctx.send(f"{str(players[i])} แพ้เจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}")
            ...
            if player_status[i]=="p":await ctx.send(f"{str(players[i])} เสมอกับเจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}")
