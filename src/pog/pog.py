...
async def check_winner(ctx, players, player_status, index_of_bighand, listscore, player_cards):
    ...
    if player_status[int(index_of_bighand)] == "p":
        ...
        await ctx.channel.send(f"เจ้ามือ{str(players[int(index_of_bighand)])}ถือไพ่ {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][0]]}   {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][1]]}   {DECKS_OF_CARDS[player_cards[int(index_of_bighand)][2]]}")
    print(players)
    print(player_status)
    print(listscore)
    print(player_cards)
    ...
    for i in range(len(players)):
        ...
        if listscore[i] > listscore[int(index_of_bighand)]:
            print("w")
            if player_status[i] == "p":
                await ctx.channel.send(f"{str(players[i])}  ชนะเจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}")
            ...
            print("l")
            if player_status[i] == "p":
                await ctx.channel.send(f"{str(players[i])}  แพ้เจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}")
            elif player_status[i] == "c":
            ...
            print("d")
            if player_status[i] == "p":
                await ctx.channel.send(f"{str(players[i])}  เสมอกับเจ้ามือ โดยถือไพ่ {DECKS_OF_CARDS[player_cards[i][0]]}   {DECKS_OF_CARDS[player_cards[i][1]]}")
            elif player_status[i] == "c":
