...
class GuildData:
    def __init__(self, GUILD_ID: int):
        ...
        try:
            ...
            self.codechannel_ids = [item['channelID']
                                    for item in self.data['codechannels']]
            try:
                self.return_msg = self.data['return_msg']
            except:
                self.return_msg = {}
        except:
            self.id = None
...
async def ListCodeChannel(ctx: discord_slash.SlashContext):
...
async def RemoveCodeChannel(ctx: discord_slash.SlashContext, channel: discord.TextChannel):
    if ctx.guild_id not in GuildIDs:
        ctx.send(
            'Server นี้ยังไม่มี Code Channel กรุณาใช้คำสั่ง `/codechannel add [channel] [language] ก่อน`')
...
async def AddReturnMsg(ctx: discord_slash.SlashContext, message: str, return_message: str):
    guilddata = GuildData(ctx.guild.id)
...
async def RemoveReturnMsg(ctx: discord_slash.SlashContext, message: str):
    ...
    return_message = guilddata.return_msg.pop(message)
